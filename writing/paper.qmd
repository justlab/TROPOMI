...
```{r tbl-cv-bins}
#| tbl-cap: The CV broken down by ground value.
pretty.cv.summary(quote(list("Ground NO~2~" = cut(y.ground,
    breaks = seq(0, max(y.ground), by = 50),
    dig.lab = 5))))
```

```{r fig-mae-by-ground}
#| fig-cap: A plot of absolute error versus the ground value.

d = d.xgb()[, .(
    y.ground,
    ae.corrected = abs(y.ground.pred - y.ground),
    ae.raw = abs(y.sat - y.ground))]
ggplot(melt(d, id = "y.ground"), aes(y.ground, value, group = variable, color = variable)) +
