...
```{r fig-mae-by-ground}
#| fig-cap: MAE within each integer group of ground values.

d = d.xgb()[, keyby = .(y.ground.group = floor(y.ground)), .(
    mae.corrected = mean(abs(y.ground.pred - y.ground)),
    mae.raw = mean(abs(y.sat - y.ground)))]
ggplot(melt(d[between(y.ground.group, 25, 250)], id = "y.ground.group")) +
    geom_line(aes(y.ground.group, value,
...
show.shaps = \(ivs)
    t(t(round(d = 2, sort(dec = T,
        colMeans(abs((if (ivs == "all")
           model.with.xgboost() else
           model.with.xgboost(ivs = setdiff(all.ivs, spacetime.ivs)))$d.shap))))))
show.shaps("all")
```
